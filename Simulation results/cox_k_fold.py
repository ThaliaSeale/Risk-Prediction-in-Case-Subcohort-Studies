...
def cox_k_fold(
    fitters, cases, subcohort,n_covariates,len_cohort, weight_fun, duration_col, event_col=None, k=5, scoring_method="log_likelihood", fitter_kwargs={}, seed=None
):  # pylint: disable=dangerous-default-value,too-many-arguments,too-many-locals
    """
    Perform cross validation on a dataset. If multiple models are provided,
    all models will train on each of the k subsets.
    ...
    subcohort["subcohort"] = True
    df = pd.concat([cases,subcohort])
    ...
    for i in range(1, k + 1):
        ...
        training_data = weight_fun(training_data[training_data["subcohort"]],training_data[training_data["subcohort"] == False],n_covariates,len(subcohort)/len_cohort).drop(columns = "subcohort")
        testing_data = weight_fun(testing_data[testing_data["subcohort"]],testing_data[testing_data["subcohort"] == False],n_covariates,len(subcohort)/len_cohort).drop(columns = "subcohort")
