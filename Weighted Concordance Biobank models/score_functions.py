...
from sksurv.metrics2 import wbrier_score, wintegrated_brier_score

def wibs(model, X_tr_test, y_tr_test, y_tr_train, control_weight, lifelines = True):
    # survival function predictions
    survs = model.predict_survival_function(X_tr_test)
    
    if lifelines == False:
        minimum = max(min(model.event_times_),min(y_tr_test["duration"])) + 0.000001
        maximum = min(max(model.event_times_),max(y_tr_test["duration"])) - 0.000001
        ...
        y_train = Surv().from_dataframe("event","duration",y_tr_train)

        y_test = Surv().from_dataframe("event","duration",y_tr_test)

        score = wintegrated_brier_score(y_train, y_test, preds, times, control_weight)
    
    else:
        # times at which to evaluate survival function
        times = survs.index[np.where((survs.index < max(y_tr_test['duration'])) & (survs.index > min(y_tr_test['duration'])))]

        preds = np.array(survs.iloc[np.where((survs.index < max(y_tr_test['duration'])) & (survs.index > min(y_tr_test['duration']))) ]).transpose()
